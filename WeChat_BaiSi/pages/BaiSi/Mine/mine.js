...
Page({
  ...
  data: {
    userImage: "",
    userName: "登录/注册",
    typelists: [{ "image":"http://pic.qqtn.com/file/2013/2015-5/2015051514475586835.png", "name":"审帖"},
  ...
  onShow: function () {
    if (app.globalData.islogin) {
      this.setData({
        userImage: app.globalData.userData.userImage,
  ...
  loginClick: function(even) {
    wx.navigateTo({
      url: (app.globalData.islogin ? '../Mine/Login/setting' : '../Mine/Login/login'),
    })
  },

  downloadClick: function(even) {
    wx.showToast({
      icon: "loading",
      title: '离线下载',
      duration: 2000,
  ...
  typeClick: function(even) {
    ...
    let title = ("你点击了第 " + (index + 1) + ' 个分类');
    let messageObject = this.data.typelists[index];
    let message = messageObject.name;

    wx.showModal({
      title: title,
      content: '你选择了 ' + message,
      showCancel: false,
      confirmText: "知道了"
    })
  }
